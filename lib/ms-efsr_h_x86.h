...
/* EFS_HASH_BLOB, PSID, CERTIFICATE_BLOB, ENCRYPTION_CERTIFICATE*, EFS_RPC_BLOB,
   ENCRYPTED_FILE_METADATA_SIGNATURE and ENCRYPTION_PROTECTOR* are already provided
   by the Windows SDK (winefs.h / winnt.h, pulled in transitively via Windows.h) —
   redeclaring their bodies here conflicts with the SDK's own definitions. */
